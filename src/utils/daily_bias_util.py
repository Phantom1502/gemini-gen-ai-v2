...
daily_bias_util.py
==================
ICT Daily Bias Generator - V2
Kế thừa logic từ ICTDailyCleanGenerator (pasted), tổ chức lại thành utility class
tương thích với kiến trúc v1 (MT5Util / ChartAnalysisUtil / GeminiTradingAgent).
"""
...
class DailyBiasUtil:
    ...
    def analyze_liquidity_lifecycles(df: pd.DataFrame, swing_window: int = 2) -> List[Dict]:
        ...
        # Tìm swing high (BSL) và swing low (SSL)
        ...
        # Cập nhật trạng thái từng đường
    ...
    def generate_daily_chart(
        ...
        """
        Vẽ biểu đồ Daily (mặc định 20 nến) theo chuẩn ICT Dark Theme.
        Nến cuối cùng = Yesterday (hôm qua).
    ...
    # ──────────────────────────────────────────────
    # 3. HELPER: Resample H1 → Daily (20 nến cuối)
    # ──────────────────────────────────────────────
    @staticmethod
    def resample_h1_to_daily(df_h1: pd.DataFrame, tail: int = 20) -> pd.DataFrame:
        """
        Resample DataFrame H1 thành Daily.
        Dùng khi lấy dữ liệu từ MT5 ở timeframe H1 rồi cần xem Daily.
        """
        df_daily = df_h1.resample('D').agg({
            'Open':  'first',
            'High':  'max',
            'Low':   'min',
            'Close': 'last'
